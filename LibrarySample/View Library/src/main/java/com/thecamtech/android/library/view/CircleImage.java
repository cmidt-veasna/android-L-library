...
public class CircleImage extends View {
    ...
    private void init(AttributeSet attrs, int defStyle) {
        ...
        final TypedArray a = getContext().obtainStyledAttributes(
                attrs, R.styleable.CircleImage, defStyle, 0);

        if (a.hasValue(R.styleable.CircleImage_drawable)) {
            mDrawable = a.getDrawable(
                    R.styleable.CircleImage_drawable);
            setDrawable(mDrawable);
        }

        mUse = a.getInt(R.styleable.CircleImage_use, EFFECT_NONE);
        mEffectSize = a.getDimensionPixelSize(R.styleable.CircleImage_effectSize, 0);
        mEffectColor = a.getColor(R.styleable.CircleImage_effectColor, Color.BLACK);
